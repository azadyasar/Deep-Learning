...
path_to_test_images = "/home/azad/Documents/WorkSpaces/Python_WS/Neural_Networks/Neural_Networks/datasets/dogs_cats/test_new"
...
class Network:
    ...
    def __init__(self, path_dict):
        ...
        self.num_filters6 = 64
        self.num_filters7 = 32
        self.num_filters8 = 32
        self.num_filters9 = 32
        self.num_filters10 = 16
        self.fc_size1 = 1024 * 2
        self.fc_size2 = 512 * 2
    ...
    def build_graph(self, is_training):
        ...
        layer_conv8, weights = self.new_conv_layer(input=layer_conv7, config={"num_input_channels":self.num_filters7, 
                                              "filter_size":self.filter_size,"num_filters":self.num_filters8, 
                                              "scope":"ConvLayer8", "init_method":"normal", 
                                              "is_training":is_training}, use_pooling=False)
        with tf.variable_scope("Conv8_Dropout"):
            layer_conv6 = tf.nn.dropout(layer_conv8, keep_prob)
        print("Constructed ", layer_conv8)

        layer_conv9, weights = self.new_conv_layer(input=layer_conv8, config={"num_input_channels":self.num_filters8, 
                                              "filter_size":self.filter_size,"num_filters":self.num_filters9, 
                                              "scope":"ConvLayer9", "init_method":"normal", 
                                              "is_training":is_training}, use_pooling=False)
        with tf.variable_scope("Conv9_Dropout"):
            layer_conv6 = tf.nn.dropout(layer_conv9, keep_prob)
        print("Constructed ", layer_conv9)

        layer_conv10, weights = self.new_conv_layer(input=layer_conv9, config={"num_input_channels":self.num_filters9, 
                                              "filter_size":self.filter_size,"num_filters":self.num_filters10, 
                                              "scope":"ConvLayer10", "init_method":"normal", 
                                              "is_training":is_training}, use_pooling=False)
        with tf.variable_scope("Conv10_Dropout"):
            layer_conv6 = tf.nn.dropout(layer_conv10, keep_prob)
        print("Constructed ", layer_conv10)



        layer_flat, self.num_features = Network.flatten_layer(layer_conv10, scope="Flatten")
    ...
    def test_network(self):
        ...
        print("Test set size: %d" % data.shape[0])
        # labels_mat = Network.convert_to_categorical(labels, self.num_classes)
        ...
        #         print("Test ex {0}-{1}: {2:1.4%}".format(start, end, acc))
        ...
        #     print("Average accuracy: {0:.4%}".format(acc_avg))
        # else:
        #     labels_mat = Network.convert_to_categorical(labels, self.num_classes)
        #     acc = sess.run(accuracy, feed_dict={x_:data, y_:labels_mat, dropout_prob:1.,
        #                                             self.is_training_pc:False})
        ...
        print("\nAcc: {0:0.5%}".format(true_counter/labels.shape[0]))
        print(result.shape)
